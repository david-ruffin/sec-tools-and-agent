...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        while True:
            ...
            paraphrased_query = get_paraphrased_query(user_query, llm)
            print(f"\nOkay, just to confirm, you're asking: '{paraphrased_query}'?")

            try:
                confirmation = input("Is this correct? (yes/no): ").lower().strip()
            except EOFError:
                 logger.info("EOF received during confirmation, exiting.")
                 break

            if confirmation == 'yes':
                logger.info("User confirmed the paraphrased query.")
                # --- Proceed with Agent Execution ---
                try:
                    # Use invoke for synchronous execution WITH the callback
                    response = agent_executor.invoke(
                        {"input": user_query}, # Use the original user_query for the agent
                        config={"callbacks": callbacks_list} # Pass callbacks here
                    )
                    # Log final response output AFTER agent execution finishes
                    logger.info(f"Agent final response output: {response.get('output', 'N/A')}")
                ...
                logger.info("User did not confirm. Asking to rephrase.")
                print("\nOkay, please rephrase your question.")
                continue # Skip agent execution and ask for input again
